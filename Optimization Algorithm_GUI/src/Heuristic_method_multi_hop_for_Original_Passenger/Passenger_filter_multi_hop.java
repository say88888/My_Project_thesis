...
import READ_TXT.Read_INPUT_TXT_Original;
...
public class Passenger_filter_multi_hop {
	...
	public static void main() throws IOException {
		...
		Read_INPUT_TXT_Original.loadData();			//讀取初始資料
		...
		for(int i=0;i<Read_INPUT_TXT_Original.getP();i++){
			
		for(int j=0;j<Read_INPUT_TXT_Original.getD();j++){
			 
			int D_S,D_E,P_S,P_E;
			D_S=Integer.parseInt(Read_INPUT_TXT_Original.getD_S_PLACE(j));										//司機起點
			D_E=Integer.parseInt(Read_INPUT_TXT_Original.getD_D_ADDRESS(j));								//司機終點
			P_S=Integer.parseInt(Read_INPUT_TXT_Original.getP_R_PLACE(i));										//乘客搭車點
			P_E=Integer.parseInt(Read_INPUT_TXT_Original.getP_D_ADDRESS(i));								//乘客下車點

...
public class time_limit  extends step2_1{
	...
	System.out.println();
		...
		for(int i=0;i<length;i++){
			 if(Order[i]>0){
				 m=Order[i]-1;
				 PT[i]=Read_INPUT_TXT_filter_Original.getP_F_R_Time(m);           //乘客提早搭車時間
			 }
			 if(Order[i]<0){
				 m=-(Order[i])-1;
				 PT[i]=Read_INPUT_TXT_filter_Original.getP_D_A_Time(m);         //乘客最晚下車時間
			 }
		 }
		...
		int k=0;
		 ...
		 Driving_time=Read_INPUT_TXT_filter_Original.getD_F_D_Time(d);
		 Delay_time=0; m=0;
		 ...
		 System.out.println("  Driving_time "+Driving_time +" getD_S_A_Time  "+Read_INPUT_TXT_filter_Original.getD_S_A_Time(d));
		
		 if(c1.time(Driving_time,Read_INPUT_TXT_filter_Original.getD_S_A_Time(d))==true && Time==true){Time=true;}
		 else{Time=false;}

...
public class V_Individual extends DMSDL_PSO2_Tourism{
    ...
    public void XvelocityIndividual() {
    	for (int i = 0; i < Vxasize(); i++) {
			Vxa[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
		}
		for (int i = 0; i < Vxhsize(); i++) {
			Vxh[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
		}
		for (int i = 0; i < Vxtsize(); i++) {
			Vxt[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
		}
    	}

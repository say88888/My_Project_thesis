...
public class U_Individual extends DMSDL_PSO2_Tourism {
	...
	public void XglobalIndividual() {
		...
		{
			Uxa[i] = PSO_Read_Write_txt.random_generate_or_read(Execution_Mode);
		}
		defaultGeneLengthXh = Xhsize;
		Uxh = new int[defaultGeneLengthXh];
		for (int i = 0; i < Uxhsize(); i++)
		{
			Uxh[i] = PSO_Read_Write_txt.random_generate_or_read(Execution_Mode);
		}
		defaultGeneLengthXt = Xtsize;
		Uxt = new int[defaultGeneLengthXt];
		for (int i = 0; i < Uxtsize(); i++)
		{
			Uxt[i] = PSO_Read_Write_txt.random_generate_or_read(Execution_Mode);
		}
	}
	...
	public void YglobalIndividual() {
		...
		{
			Uy[i] = PSO_Read_Write_txt.random_generate_or_read(Execution_Mode);
		}

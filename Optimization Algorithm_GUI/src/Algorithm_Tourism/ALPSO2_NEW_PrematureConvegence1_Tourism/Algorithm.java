...
public class Algorithm extends ALPSO2_NEW_PrematureConvegence1_Tourism {
	...
	public static Population evolvePopulation(Population pop)   {
		...
		if (Prob_adjust > r) {
			...
			for (int i = 0; i < pop.size(); i++) {
				...
				for (int d = 0; d < CandidateV.CandidateVxasize(); d++) {
					CandidateV.setCandidateVxa(d,
							w * CandidateV.getCandidateVxa(d) + (c1 * r1 * (Pinvid.getPxa(d) - invid.getXa(d)))
									+ c2 * r2 * (CandidateV.getCandidateVxa(d) - invid.getXa(d)));
					if (CandidateV.getCandidateVxa(d) > Vmax)
						CandidateV.setCandidateVxa(d, Vmax);
					if (CandidateV.getCandidateVxa(d) < -Vmax)
						CandidateV.setCandidateVxa(d, -Vmax);
				}
				for (int d = 0; d < CandidateV.CandidateVxhsize(); d++) {
					CandidateV.setCandidateVxh(d,
							w * CandidateV.getCandidateVxh(d) + (c1 * r1 * (Pinvid.getPxh(d) - invid.getXh(d)))
									+ c2 * r2 * (CandidateV.getCandidateVxh(d) - invid.getXh(d)));
					if (CandidateV.getCandidateVxh(d) > Vmax)
						CandidateV.setCandidateVxh(d, Vmax);
					if (CandidateV.getCandidateVxh(d) < -Vmax)
						CandidateV.setCandidateVxh(d, -Vmax);
				}
				for (int d = 0; d < CandidateV.CandidateVxtsize(); d++) {
					CandidateV.setCandidateVxt(d,
							w * CandidateV.getCandidateVxt(d) + (c1 * r1 * (Pinvid.getPxt(d) - invid.getXt(d)))
									+ c2 * r2 * (CandidateV.getCandidateVxt(d) - invid.getXt(d)));
					if (CandidateV.getCandidateVxt(d) > Vmax)
						CandidateV.setCandidateVxt(d, Vmax);
					if (CandidateV.getCandidateVxt(d) < -Vmax)
						CandidateV.setCandidateVxt(d, -Vmax);
				}
				...
				for (int d = 0; d < invid.Xasize(); d++) {
					...
					if (rsid < 1 / (1 + Math.exp(-PSO_lamda * CandidateV.getCandidateVxa(d)))) {
						Candidate.setXa(d, 1);
					} else {
						Candidate.setXa(d, 0);
					}
				...
				for (int d = 0; d < invid.Xhsize(); d++) {
					...
					if (rsid < 1 / (1 + Math.exp(-PSO_lamda * CandidateV.getCandidateVxh(d)))) {
						Candidate.setXh(d, 1);
					} else {
						Candidate.setXh(d, 0);
					}
				...
				for (int d = 0; d < invid.Xtsize(); d++) {
					...
					if (rsid < 1 / (1 + Math.exp(-PSO_lamda * CandidateV.getCandidateVxt(d)))) {
						Candidate.setXt(d, 1);
					} else {
						Candidate.setXt(d, 0);
					}
				...
				for (int d = 0; d < invid.Ysize(); d++) {
					double rsid = PSO_Read_Write_txt.random_generate_or_read1(Execution_Mode);
					if (rsid < 1 / (1 + Math.exp(-PSO_lamda * CandidateV.getCandidateVy(d)))) {
						Candidate.setY(d, 1);
					} else {
						Candidate.setY(d, 0);
					}
	...
	static CandidateV_Individual GenerateCandidate(Population pop, int i, CandidateV_Individual CandidateV)   {
		...
		for (int d = 0; d < Xasize; d++) {
			...
			if (Prob_candidate > rgen) {
				...
				CandidateV.setCandidateVxa(d, GVinvid.getGVxa(d));
				...
				if (Pinvid1.getFitness() > Pinvid2.getFitness()) {
					CandidateV.setCandidateVxa(d, Vinvid1.getVxa(d) + Gaussian_xa(pop, d));
				} else {
					CandidateV.setCandidateVxa(d, Vinvid2.getVxa(d) + Gaussian_xa(pop, d));
				}
			}
		}
		for (int d = 0; d < Xhsize; d++) {
			...
			if (Prob_candidate > rgen) {
				...
				CandidateV.setCandidateVxh(d, GVinvid.getGVxh(d));
				...
				if (Pinvid1.getFitness() > Pinvid2.getFitness()) {
					CandidateV.setCandidateVxh(d, Vinvid1.getVxh(d) + Gaussian_xh(pop, d));
				} else {
					CandidateV.setCandidateVxh(d, Vinvid2.getVxh(d) + Gaussian_xh(pop, d));
				}
			}
		}
		for (int d = 0; d < Xtsize; d++) {
			...
			if (Prob_candidate > rgen) {
				...
				CandidateV.setCandidateVxt(d, GVinvid.getGVxt(d));
				...
				if (Pinvid1.getFitness() > Pinvid2.getFitness()) {
					CandidateV.setCandidateVxt(d, Vinvid1.getVxt(d) + Gaussian_xt(pop, d));
				} else {
					CandidateV.setCandidateVxt(d, Vinvid2.getVxt(d) + Gaussian_xt(pop, d));
				}
			}
		}

...
public class V_Individual extends ALPSO2_NEW_PrematureConvegence1_Tourism {
	...
	public void XvelocityIndividual() {
		...
		{
			Vxa[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);// Math.random()*(Vmax-(-Vmax))+(-Vmax);
		}
		for (int i = 0; i < Vxhsize(); i++)
		{
			Vxh[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);// Math.random()*(Vmax-(-Vmax))+(-Vmax);
		}
		for (int i = 0; i < Vxtsize(); i++)
		{
			Vxt[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);// Math.random()*(Vmax-(-Vmax))+(-Vmax);
		}
	}

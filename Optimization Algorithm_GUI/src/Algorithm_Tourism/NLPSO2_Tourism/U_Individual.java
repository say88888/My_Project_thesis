...
public class U_Individual extends NLPSO2_Tourism{
	...
	public void XglobalIndividual() {
		...
		for (int i = 0; i < Uxasize(); i++) {
			Uxa[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
			double rsid = PSO_Read_Write_txt.random_generate_or_read1(Execution_Mode);
			if (rsid < 1 / (1 + Math.exp(-PSO_lamda * Uxa[i]))) {
				binaryUxa[i] = 1;
			} else {
				binaryUxa[i] = 0;
			}
		...
		for (int i = 0; i < Uxhsize(); i++) {
			Uxh[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
			double rsid = PSO_Read_Write_txt.random_generate_or_read1(Execution_Mode);
			if (rsid < 1 / (1 + Math.exp(-PSO_lamda * Uxh[i]))) {
				binaryUxh[i] = 1;
			} else {
				binaryUxh[i] = 0;
			}
		...
		for (int i = 0; i < Uxtsize(); i++) {
			Uxt[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
			double rsid = PSO_Read_Write_txt.random_generate_or_read1(Execution_Mode);
			if (rsid < 1 / (1 + Math.exp(-PSO_lamda * Uxt[i]))) {
				binaryUxt[i] = 1;
			} else {
				binaryUxt[i] = 0;
			}
	...
	public void YglobalIndividual() {
		...
		for (int i = 0; i < Uysize(); i++) {
			Uy[i] = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
			double rsid = PSO_Read_Write_txt.random_generate_or_read1(Execution_Mode);
			if (rsid < 1 / (1 + Math.exp(-PSO_lamda * Uy[i]))) {
				binaryUy[i] = 1;
			} else {
				binaryUy[i] = 0;
			}

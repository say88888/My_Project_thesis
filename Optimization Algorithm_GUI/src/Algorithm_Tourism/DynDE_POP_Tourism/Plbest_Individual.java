...
public class Plbest_Individual extends DynDE_POP_Tourism {
    ...
    public void XglobalIndividual() {
		...
		for (int i = 0; i < Pxasize(); i++) {
			Pxa[i] = DE_Read_Write_txt.random_generate_or_read4(Execution_Mode);
			double rsid = DE_Read_Write_txt
					.random_generate_or_read1(Execution_Mode);
			if (rsid < 1 / (1 + Math.exp(-lamda * Pxa[i]))) {
				binaryPxa[i] = 1;
			} else {
				binaryPxa[i] = 0;
			}
		...
		for (int i = 0; i < Pxhsize(); i++) {
			Pxh[i] = DE_Read_Write_txt.random_generate_or_read4(Execution_Mode);
			double rsid = DE_Read_Write_txt
					.random_generate_or_read1(Execution_Mode);
			if (rsid < 1 / (1 + Math.exp(-lamda * Pxh[i]))) {
				binaryPxh[i] = 1;
			} else {
				binaryPxh[i] = 0;
			}
		...
		for (int i = 0; i < Pxtsize(); i++) {
			Pxt[i] = DE_Read_Write_txt.random_generate_or_read4(Execution_Mode);
			double rsid = DE_Read_Write_txt
					.random_generate_or_read1(Execution_Mode);
			if (rsid < 1 / (1 + Math.exp(-lamda * Pxt[i]))) {
				binaryPxt[i] = 1;
			} else {
				binaryPxt[i] = 0;
			}

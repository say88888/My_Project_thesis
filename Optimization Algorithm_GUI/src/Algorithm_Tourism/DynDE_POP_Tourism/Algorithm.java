...
public class Algorithm extends DynDE_POP_Tourism {
	...
	public static Population evolvePopulation(Population pop)   {
		...
		for (int i = 0; i < n3.size(); i++) {
			...
			for (int j = 0; j < n3.get(i).size(); j++) {
				Individual invid1 = pop.getIndividual((i*(pop.size()/S)+j));
				if (invid1.getFitness1() > Plbestinvid1.getFitness()) {
					for (int d = 0; d < Plbestinvid1.Pxasize(); d++) {
						Plbestinvid1.setbinaryPxa(d, invid1.getbinaryXa(d));
						Plbestinvid1.setPxa(d, invid1.getXa(d));
					}
					for (int d = 0; d < Plbestinvid1.Pxhsize(); d++) {
						Plbestinvid1.setbinaryPxh(d, invid1.getbinaryXh(d));
						Plbestinvid1.setPxh(d, invid1.getXh(d));
					}
					for (int d = 0; d < Plbestinvid1.Pxtsize(); d++) {
						Plbestinvid1.setbinaryPxt(d, invid1.getbinaryXt(d));
						Plbestinvid1.setPxt(d, invid1.getXt(d));
					}
		...
		for (int i = 0; i < n3.size(); i++) {
			...
			if (Plbestinvid1.getFitness() > Ginvid1.getFitness1()) {
				for (int d = 0; d < Plbestinvid1.Pxasize(); d++){
					...
					Ginvid1.setbinaryGxa(d, Plbestinvid1.getbinaryPxa(d));
				}
				for (int d = 0; d < Plbestinvid1.Pxhsize(); d++){
					Ginvid1.setGxh(d, Plbestinvid1.getPxh(d));
					Ginvid1.setbinaryGxh(d, Plbestinvid1.getbinaryPxh(d));
				}
				for (int d = 0; d < Plbestinvid1.Pxtsize(); d++){
					Ginvid1.setGxt(d, Plbestinvid1.getPxt(d));
					Ginvid1.setbinaryGxt(d, Plbestinvid1.getbinaryPxt(d));
				}
				for (int d = 0; d < Plbestinvid1.Pysize(); d++){
					...
					if (Function.calculateDistance(Plbestinvid_i, Plbestinvid_j) < Dthreshold) {
						...
						if(Type.get(i)==1){
							for (int k = 0; k < n3.get(i).size(); k++) {
								...
								newPopulation.saveU_Individual((i*(pop.size()/S)+k), newUinvid);
							}
						} if(Type.get(i)==2){
							for (int k = 0; k < n3.get(i).size(); k++) {
								Individual invid = pop.getIndividual((i*(pop.size()/S)+k));
								U_Individual newUinvid = new U_Individual();
						...
						if(Type.get(j)==1){
							for (int k = 0; k < n3.get(j).size(); k++) {
								...
								U_Individual newUinvid = new U_Individual();
								newUinvid = Function.Algorithm2( invid, newPinvid);
								newPopulation.saveU_Individual((j*(pop.size()/S)+k), newUinvid);
							}
						} if(Type.get(j)==2){
							for (int k = 0; k < n3.get(j).size(); k++) {
								Individual invid = pop.getIndividual((j*(pop.size()/S)+k));
								U_Individual newUinvid = new U_Individual();
								newUinvid=Function.Entropy_Differential_Evolution(invid);
								newPopulation.saveU_Individual((j*(pop.size()/S)+k), newUinvid);
							}

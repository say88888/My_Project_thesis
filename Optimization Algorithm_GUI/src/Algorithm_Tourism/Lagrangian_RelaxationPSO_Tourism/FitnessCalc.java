...
public class FitnessCalc extends LRPSO_Tourism {
	...
	static double Fitness1(int[][] Xa,int[][] Xh,int[][] Xt, int[][] Y )   {
		...
		ArrayList<List<Integer>> g=Function_Vector.g_Vector();
		ArrayList<Integer> z=Function_Vector.z_Vector(Y, Xa, Xh, Xt);
		...
		for(int i=0;i<g.size();i++){
			...
			for(int j=0;j<g.get(i).size();j++){
				sum+=g.get(i).get(j)*z.get(j);
				 
			}
			fitness+=Lambda.get(i)*Math.max(sum+B.get(i), 0);
		}

package Algorithm_Tourism.DMS_L_PSO2_Tourism;

public class Algorithm extends DMS_L_PSO2_Tourism {
	...
	public static Population evolvePopulation(Population pop) {
		...
		for (int i = 0; i < pop.size(); i++) {
			...
			P_Individual Pinvid = pop.getP_Individual(i);
			Plbest_Individual PlbestPinvid = pop.getPlbest_Individual(i);
			...
			for (int d = 0; d < Vinvid.Vxasize(); d++) {
				Vinvid.setVxa(d, w * Vinvid.getVxa(d) + (c1 * r1 * (Pinvid.getPxa(d) - invid.getXa(d))) + c2 * r2 * (PlbestPinvid.getPxa(d) - invid.getXa(d)));
			...
			for (int d = 0; d < Vinvid.Vxhsize(); d++) {
				Vinvid.setVxh(d, w * Vinvid.getVxh(d) + (c1 * r1 * (Pinvid.getPxh(d) - invid.getXh(d))) + c2 * r2 * (PlbestPinvid.getPxh(d) - invid.getXh(d)));
			...
			for (int d = 0; d < Vinvid.Vxtsize(); d++) {
				Vinvid.setVxt(d, w * Vinvid.getVxt(d) + (c1 * r1 * (Pinvid.getPxt(d) - invid.getXt(d))) + c2 * r2 * (PlbestPinvid.getPxt(d) - invid.getXt(d)));
			...
			for (int d = 0; d < Vinvid.Vysize(); d++) {
				Vinvid.setVy(d, w * Vinvid.getVy(d) + (c1 * r1 * (Pinvid.getPy(d) - invid.getY(d))) + c2 * r2 * (PlbestPinvid.getPy(d) - invid.getY(d)));

...
public class Algorithm extends L_SHADE_CCPSO2_Tourism {
		...
		for (int j = 0; j < n3.size(); j++) {
			for (int i = 0; i < pop.size(); i++) {
				...
				if (FitnessCalc.F(xa,xh,xt, y)>FitnessCalc.F(pxa,pxh,pxt, py)) {		//比較F(b(j,Pj.xi)) > F(b(j,Pj.yi))
					...
					ferc.add(Math.abs(FitnessCalc.F(xa,xh,xt, y)-FitnessCalc.F(pxa,pxh,pxt, py)));
					if (Print_Mode == 1)
			...
			if (FitnessCalc.F(gxa,gxh,gxt, gy) > Ginvid2.getFitness1()) {	////比較 F(b(j,Pj.^yi))>F(^y)
				if (Print_Mode == 1)
					System.out.println(" F(b(Gx,Gy))=" + FitnessCalc.F(pxa,pxh,pxt, py)+ " F(Gx,Gy)=" + FitnessCalc.F(gxa,gxh,gxt, gy)+ "F(b(Gx,Gy))>F(Gx,Gy) Gx=Gx,Gy=Gy");
				for (int k = 0; k < n3.get(j).size(); k++) {
			...
			for (int i = 0; i < pop.size(); i++) {
				...
				P_Individual Pinvid = pop.getP_Individual(i);
				G_Individual Ginvid = pop.getG_Individual();
				int r1=PSO_Read_Write_txt.random_generate_or_read5(Execution_Mode, H);
				Alpha1[i]=PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*0.1+MAlpha1[r1];
				Alpha2[i]=PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*0.1+MAlpha2[r1];
				Alpha3[i]=PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*0.1+MAlpha3[r1];
			
				for (int d = 0; d < Vinvid.Vxasize(); d++) {	//變更X速度
					if (Print_Mode == 1)
						System.out.print("Vx=" + Vinvid.getVxa(d) + " ");
					Vinvid.setVxa(d,PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*Alpha3[i]*Math.abs(Pinvid.getPxa(d)-Ginvid.getGxa(d))
							+(Alpha1[i]*Pinvid.getPxa(d)+Alpha2[i]*Ginvid.getGxa(d)));			
					if (Vinvid.getVxa(d) > Vmax)
						Vinvid.setVxa(d, Vmax);
					if (Vinvid.getVxa(d) < -Vmax)
						Vinvid.setVxa(d, -Vmax);
					if (Print_Mode == 1)
						System.out.println("改為 Vx=" + Vinvid.getVxa(d) + " ");

				}
				for (int d = 0; d < Vinvid.Vxhsize(); d++) {	//變更X速度
					if (Print_Mode == 1)
						System.out.print("Vx=" + Vinvid.getVxh(d) + " ");
					Vinvid.setVxh(d,PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*Alpha3[i]*Math.abs(Pinvid.getPxh(d)-Ginvid.getGxh(d))
							+(Alpha1[i]*Pinvid.getPxh(d)+Alpha2[i]*Ginvid.getGxh(d)));			
					if (Vinvid.getVxh(d) > Vmax)
						Vinvid.setVxh(d, Vmax);
					if (Vinvid.getVxh(d) < -Vmax)
						Vinvid.setVxh(d, -Vmax);
					if (Print_Mode == 1)
						System.out.println("改為 Vx=" + Vinvid.getVxh(d) + " ");

				}
				for (int d = 0; d < Vinvid.Vxtsize(); d++) {	//變更X速度
					if (Print_Mode == 1)
						System.out.print("Vx=" + Vinvid.getVxt(d) + " ");
					Vinvid.setVxt(d,PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*Alpha3[i]*Math.abs(Pinvid.getPxt(d)-Ginvid.getGxt(d))
							+(Alpha1[i]*Pinvid.getPxt(d)+Alpha2[i]*Ginvid.getGxt(d)));			
					if (Vinvid.getVxt(d) > Vmax)
						Vinvid.setVxt(d, Vmax);
					if (Vinvid.getVxt(d) < -Vmax)
						Vinvid.setVxt(d, -Vmax);
					if (Print_Mode == 1)
						System.out.println("改為 Vx=" + Vinvid.getVxt(d) + " ");

				}
				for (int d = 0; d < Vinvid.Vysize(); d++) {	//變更Y速度
					if (Print_Mode == 1)
						System.out.print("Vy=" + Vinvid.getVy(d) + " ");
					Vinvid.setVy(d,PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*Alpha3[i]*Math.abs(Pinvid.getPy(d)-Ginvid.getGy(d))
							+(Alpha1[i]*Pinvid.getPy(d)+Alpha2[i]*Ginvid.getGy(d)));
			...
			for (int i = 0; i < pop.size(); i++) {
				...
				for (int d = 0; d < invid.Xasize(); d++) {		//變更X粒子
					...
					if (rsid < 1 / (1 + Math.exp(-PSO_lamda*Vinvid.getVxa(d)))) {
						if (Print_Mode == 1)
							System.out.print("rsid<S(Vx)  Xa為1");
						invid.setXa(d, 1);
					} else {
						if (Print_Mode == 1)
							System.out.print("rsid>=S(Vx)  Xa為0");
						invid.setXa(d, 0);
					}
					if (Print_Mode == 1)
						System.out.println();
				...
				for (int d = 0; d < invid.Xhsize(); d++) {		//變更X粒子
					...
					if (rsid < 1 / (1 + Math.exp(-PSO_lamda*Vinvid.getVxh(d)))) {
						if (Print_Mode == 1)
							System.out.print("rsid<S(Vx)  Xh為1");
						invid.setXh(d, 1);
					} else {
						if (Print_Mode == 1)
							System.out.print("rsid>=S(Vx)  Xh為0");
						invid.setXh(d, 0);
					}
					if (Print_Mode == 1)
						System.out.println();
				...
				for (int d = 0; d < invid.Xtsize(); d++) {		//變更X粒子
					...
					if (rsid < 1 / (1 + Math.exp(-PSO_lamda*Vinvid.getVxt(d)))) {
						if (Print_Mode == 1)
							System.out.print("rsid<S(Vx)  Xt為1");
						invid.setXt(d, 1);
					} else {
						if (Print_Mode == 1)
							System.out.print("rsid>=S(Vx)  Xt為0");
						invid.setXt(d, 0);
					}
					if (Print_Mode == 1)
						System.out.println();

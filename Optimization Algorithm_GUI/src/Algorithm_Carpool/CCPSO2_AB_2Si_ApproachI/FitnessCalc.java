...
public class FitnessCalc extends CCPSO2_AB_2Si_ApproachI {
	...
	static double F(int[] A, int[] B) {
		...
		for (int d = 0; d < D; d++) {
			...
			for (int i = 0; i < B.length; i++)
				for(int j=0;j<Dlist.get(d).size();j++){
					if (Math.abs(B[i]) == (Dlist.get(d).get(j)))
							B1.add(B[i]);
					}
			Bdm.add(B1);
		}
		...
		// U1(X,Y)
			...
			double Sfmin;
			Sfmin = CCPSO2_AB_2Si_ApproachI.Sfmin;
	...
	static double getEFitness(E_Individual individual) {

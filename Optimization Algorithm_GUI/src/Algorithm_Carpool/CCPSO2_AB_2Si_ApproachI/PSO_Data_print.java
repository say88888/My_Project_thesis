...
public class PSO_Data_print {
	public static void print2(Population pop,int Print_Mode) {
		if(Print_Mode==1){
			...
			for(int j=0;j<pop.getFittest().binaryGxsize();j++)
	   			 System.out.print(pop.getFittest().getbinaryGx(j));
			System.out.println();
			System.out.print("Gy=");
			 
	    		for(int j=0;j<pop.getFittest().binaryGysize();j++)
	    			 System.out.print(pop.getFittest().getbinaryGy(j));

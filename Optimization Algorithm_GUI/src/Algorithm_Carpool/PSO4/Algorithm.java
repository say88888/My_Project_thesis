...
public class Algorithm extends PSO4 {
	...
	public static Population evolvePopulation(Population pop) {
		...
		for (int i = 0; i < pop.size(); i++) {
			...
			for (int d = 0; d < Vinvid.Vxsize(); d++) {
				...
				Vinvid.setVx(d,Double.parseDouble(nf.format(w * Vinvid.getVx(d)+ (c1 * r1 * (Pinvid.getPx(d) - invid.getX(d)))+ c2 * r2 * (Ginvid.getGx(d) - invid.getX(d)))));
			...
			for (int d = 0; d < Vinvid.Vysize(); d++) {
				...
				Vinvid.setVy(d,Double.parseDouble(nf.format(w * Vinvid.getVy(d)+ (c1 * r1 * (Pinvid.getPy(d) - invid.getY(d)))+ c2 * r2 * (Ginvid.getGy(d) - invid.getY(d)))));
		...
		for(int i=0;i<pop.size();i++){
			...
			if(Print_Mode==1)
				System.out.println("第"+i+"個");
			for(int d=0;d<invid.Xsize();d++){
				invid.setX(d, invid.getX(d)+Vinvid.getVx(d));
				if(Print_Mode==1)
					System.out.println("X= "+invid.getX(d));
		}
			for(int d=0;d<invid.Ysize();d++){
				invid.setY(d, invid.getY(d)+Vinvid.getVy(d));
					if(Print_Mode==1)
						System.out.println("Y= "+invid.getY(d));

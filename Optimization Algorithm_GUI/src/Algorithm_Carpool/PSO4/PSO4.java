...
public class PSO4 {
	...
	public static void main(  ) throws IOException {
		...
		epsilon=PSO_Read_Write_txt.epsilon();
		micro=PSO_Read_Write_txt.micro();
		...
		System.out.print("Generation: " + generationCount +  " F(x,y) Fittest: "+ myPop.getFittest().getFitness() + " F4(x,y) Fittest: "+ myPop.getFittest().getFitness1() + " Y=");
	...
	//	for(int t=0;t<10000;t++){
		...
		Output_Result.PSO4(Y, X, Fittest, generationCount, T_total_time, (T_total_time-time_write_to_TXT),str);
		}else{
			...
			epsilon=Read_PSO_Batch_Parameter.epsilon();
			micro=Read_PSO_Batch_Parameter.micro();
			for(populationSize=Read_PSO_Batch_Parameter.lowpopulationSize();populationSize<=Read_PSO_Batch_Parameter.UppopulationSize();populationSize+=Read_PSO_Batch_Parameter.StpopulationSize()){
				for(w=Read_PSO_Batch_Parameter.loww();w<=Read_PSO_Batch_Parameter.Upw();w+=Read_PSO_Batch_Parameter.Stw()){
					for(c1=Read_PSO_Batch_Parameter.lowc1();c1<=Read_PSO_Batch_Parameter.Upc1();c1+=Read_PSO_Batch_Parameter.Stc1()){
					 ...
					 sb=new StringBuilder();
					 sb1=new StringBuilder();
					 	System.out.println("PSO4_Batch"+n);
		...
		//	for(int t=0;t<100;t++){
			...
			CheckConstraint.CheckConstraints();
			Print_input.main(Print_Mode);
			Output_Result.savePSO4_Batch_Parameter(n,populationSize,w,c1,c2, Vmax, Execution_Mode, Print_Mode, threshold,epsilon,micro );
			Output_Result.PSO4_Batch(n,Y, X, Fittest, generationCount,T_total_time, (T_total_time-time_write_to_TXT));
			System.out.println();
		...
		/*		Time_Batch=endTime-startTime;
				generationCount_Batch=generationCount;
			
				populationSize_Batch=populationSize;
				w_Batch=w;
				c1_Batch=c1;
				c2_Batch=c2;
				
				for (int i = 0; i < myPop.getFittest().Gysize(); i++) {
					Y_Batch[i] = myPop.getFittest().getGy(i);
				}
				System.out.print("X=");
				for (int j = 0; j < myPop.getFittest().Gxsize(); j++) {
					X_Batch[j] = myPop.getFittest().getGx(j);
				}
			}*/
						}
					}

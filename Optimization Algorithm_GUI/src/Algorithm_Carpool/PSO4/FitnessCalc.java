...
public class FitnessCalc extends PSO4{
...
//F2(X,Y)
    static double getFitness1(Individual individual) {
		...
		double st221=0,st222=0;
		 ...
		 for(int d=0;d<D;d++)
			 for(int j=0;j<X[d].length;j++){
				 if((X[d][j]>=0 && X[d][j]<=epsilon) || ((X[d][j]>=(1-epsilon))&& X[d][j]<=1)){
				 }
				 else{
					 right=false;
				 }
		 ...
		 for(int p=0;p<P;p++)
			 for(int h=0;h<Y[p].length;h++){
				 if((Y[p][h]>=0 && Y[p][h]<=epsilon) || (Y[p][h]>=(1-epsilon) && Y[p][h]<=1)){
				 }
				 else{
					 right=false;
				 }
    ...
    static double penalty(double XY){
    	double a=0;
    	if(XY>=epsilon && XY<=(1-epsilon))
    		a=-micro;
    	else
    		a=0;
    	return a;

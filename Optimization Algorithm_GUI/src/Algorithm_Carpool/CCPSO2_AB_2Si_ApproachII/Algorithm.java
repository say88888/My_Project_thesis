...
public class Algorithm extends CCPSO2_AB_2Si_ApproachII {
	...
	public static Population evolvePopulation(Population pop) {
		...
		for (int j = 0; j < n3.size(); j++) {
			for (int i = 0; i < pop.size(); i++) {
				...
				for(int d=0;d<P;d++){				//Gx先存放在x、px
					x[d]=Ginvid.getGx(d);
					px[d]=Ginvid.getGx(d);
				}
				for(int d=0;d<P;d++){				//Gy先存放在y、py
					y[d]=Ginvid.getGy(d);
					py[d]=Ginvid.getGy(d);
				}
				for (int k = 0; k < n3.get(j).size(); k++) {	//根據分群來產生b(j,Pj.Xi)、b(j,Pj.Yi)
					if (n3.get(j).get(k) < P) {
						...
						for (int g = 0; g < P; g++)
							if (g == n3.get(j).get(k) - P) {	//如果gy為當前分群中挑選的維度時
				...
				if (FitnessCalc.F(A, B)>FitnessCalc.F(PA, PB)) {		//比較F(b(j,Pj.xi)) > F(b(j,Pj.yi))
					...
					for (int k = 0; k < n3.get(j).size(); k++) {
						if (n3.get(j).get(k) < P) {			
							for (int g = 0; g < P; g++)						//如果F(b(j,Pj.xi)) > F(b(j,Pj.yi))時
								if (g == n3.get(j).get(k)) {
									Pinvid.setPx(g, x[g]);							//b(j,Pj.xi))取代b(j<Pj.yi)
									}
								} else {
							for (int g = 0; g < P; g++)
								if (g == n3.get(j).get(k) - P) 
									Pinvid.setPy(g, y[g]);
						}
					}
				...
				for (int k = 0; k < n3.get(j).size(); k++) {	//根據分群來產生b(j,Pj.Yi)、b(j,Pj.^Yi)
					if (n3.get(j).get(k) < P) {
						...
						for (int g = 0; g < P; g++)
							if (g == n3.get(j).get(k) - P) {	//如果g為當前分群中挑選的維度時
				...
				if (FitnessCalc.F(PA, PB) > FitnessCalc.F(GA, GB)) {		//比較F(b(j,Pj.yi)) > F(b(j,Pj.^yi))
					if (Print_Mode == 1)
						System.out.println(" F(b(Px,Py))=" + FitnessCalc.F(GA, GB)+ " F(b(Gx,Gy))=" + FitnessCalc.F(GA, GB)+ "F(b(Px,Py))>F(b(Gx,Gy)) Gx=Px,Gy=Py");
					for (int k = 0; k < n3.get(j).size(); k++) {
						if (n3.get(j).get(k) < P) {								//如果F(b(j,Pj.yi)) > F(b(j,Pj.^yi))時
							for (int g = 0; g < P; g++)
								if (g == n3.get(j).get(k)) {
			...
			if (FitnessCalc.F(GA, GB) > Ginvid2.getFitness1()) {	////比較 F(b(j,Pj.^yi))>F(^y)
				if (Print_Mode == 1)
					System.out.println(" F(b(Gx,Gy))=" + FitnessCalc.F(PA, PB)+ " F(Gx,Gy)=" + Ginvid2.getFitness1()+ "F(b(Gx,Gy))>F(Gx,Gy) Gx=Gx,Gy=Gy");
				for (int k = 0; k < n3.get(j).size(); k++) {
					if (n3.get(j).get(k) < P) {							///如果F(b(j,Pj.^yi))>F(^y)時
						for (int g = 0; g < P; g++)
							if (g == n3.get(j).get(k)) 
			...
			for (int i = 0; i < pop.size(); i++) {
				...
				for (int d = 0; d < P; d++) {	//變更X速度
					if (Print_Mode == 1)
						System.out.print("X=" + invid.getX(d) + " ");
					invid.setX(d,PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*Alpha3*Math.abs(Pinvid.getPx(d)-Ginvid.getGx(d))+(Alpha1*Pinvid.getPx(d)+Alpha2*Ginvid.getGx(d)));
				...
				for (int d = 0; d < P; d++) {	//變更Y速度
					if (Print_Mode == 1)
						System.out.print("Y=" + invid.getY(d) + " ");
					invid.setY(d,PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*Alpha3*Math.abs(Pinvid.getPy(d)-Ginvid.getGy(d))+(Alpha1*Pinvid.getPy(d)+Alpha2*Ginvid.getGy(d)));
					if (invid.getY(d) > Vmax)
						invid.setY(d, Vmax);
					if (invid.getY(d) < -Vmax)
						invid.setY(d, -Vmax);
					if (Print_Mode == 1)
						System.out.println("改為 Y=" + invid.getY(d) + " ");
				}

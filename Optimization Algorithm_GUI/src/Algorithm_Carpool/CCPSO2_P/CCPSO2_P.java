...
public class CCPSO2_P {
	...
	public static void main(  ) throws IOException {
		...
		Output_Result.CCPSO2_P(Y, X, Fittest, Fittest_Iter, T_total_time, (T_total_time-time_write_to_TXT));
		String X1="",Y1="";
		for(int i=0;i<X.length;i++)
				X1+=X[i]+" ";
		for(int i=0;i<Y.length;i++)
		...
		//	for(int t=0;t<100;t++){
			...
			CheckConstraint.CheckConstraints();
			Print_input.main(Print_Mode);
			Output_Result.saveCCPSO2_P_Batch_Parameter(n,populationSize,w,c1,c2, Vmax, Execution_Mode, Print_Mode, threshold );
			Output_Result.CCPSO2_P_Batch(n,Y, X, Fittest, generationCount,T_total_time, (T_total_time-time_write_to_TXT));
			System.out.println();

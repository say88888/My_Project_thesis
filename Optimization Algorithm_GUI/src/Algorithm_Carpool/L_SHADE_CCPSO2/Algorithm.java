...
public class Algorithm extends L_SHADE_CCPSO2 {
		...
		for (int j = 0; j < n3.size(); j++) {
			for (int i = 0; i < pop.size(); i++) {
				...
				if (FitnessCalc.F(x, y)>FitnessCalc.F(px, py)) {		//比較F(b(j,Pj.xi)) > F(b(j,Pj.yi))
					...
					ferc.add(Math.abs(FitnessCalc.F(x, y)-FitnessCalc.F(px, py)));
					if (Print_Mode == 1)
			...
			for (int i = 0; i < pop.size(); i++) {
				...
				for (int d = 0; d < Vinvid.Vxsize(); d++) {	//變更X速度
					...
					Vinvid.setVx(d,PSO_Read_Write_txt.random_generate_or_read3(Execution_Mode)*Alpha3[i]*Math.abs(Pinvid.getPx(d)-Ginvid.getGx(d))
							+(Alpha1[i]*Pinvid.getPx(d)+Alpha2[i]*Ginvid.getGx(d)));			

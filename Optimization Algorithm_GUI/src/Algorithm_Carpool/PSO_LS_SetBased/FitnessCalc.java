...
public class FitnessCalc extends PSO_LS_SetBased {
	...
	static double getFitness(G_Individual individual) {
		...
		for (int d = 0; d < D; d++) {
			...
			for (int i = 0; i < individual.GBsize1(d); i++){
					if(individual.getGB(d,i)!=0)
						B.add(individual.getGB(d, i));
				}
	...
	static double getFitness(Individual individual) {
		...
		for (int d = 0; d < D; d++) {
			List<Integer> B = new ArrayList<Integer>();
			for (int i = 0; i < individual.Bsize1(d); i++)
				if(individual.getB(d,i)!=0){
				B.add(individual.getB(d, i));
				}
			Bdm.add(B);
		}
		int[][][] Cdmp = new int[D][][];
	...
	static double getPFitness(P_Individual individual) {
		...
		for (int d = 0; d < D; d++) {
			List<Integer> B = new ArrayList<Integer>();
			for (int i = 0; i < individual.PBsize1(d); i++)
				if(individual.getPB(d,i)!=0){
				B.add(individual.getPB(d, i));
				}
			Bdm.add(B);
		}
	...
	static double getEFitness1(E_Individual individual) {
		...
		for (int d = 0; d < D; d++) {
			List<Integer> B = new ArrayList<Integer>();
			for (int i = 0; i < individual.EBsize1(d); i++)
				if(individual.getEB(d,i)!=0){
					B.add(individual.getEB(d, i));
				}
			Bdm.add(B);
		}

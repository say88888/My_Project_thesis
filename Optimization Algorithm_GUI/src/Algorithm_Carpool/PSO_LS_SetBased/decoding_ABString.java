...
public class decoding_ABString extends PSO_LS_SetBased{
	...
	public	static void main(Population pop) throws IOException
	{
		decoding_ABString(pop);
		...
		Output_Result.PSO_LS_SetBased_decodingABString(X,Y,Bids,Bdm);
		
	}

	static void decoding_ABString(Population pop)
	{
		...
		for (int d = 0; d < pop.getFittest().GAsize(); d++)
			for (int p = 0; p < pop.getFittest().GAsize1(d); p++)
			System.out.print(pop.getFittest().getGA(d,p)+" ");
		System.out.println();
		System.out.println("-----------------------------");
		
		System.out.println("--------BString---------------------");
		for (int d = 0; d < pop.getFittest().GBsize(); d++)
			for (int p = 0; p < pop.getFittest().GBsize1(d); p++)
			System.out.print(pop.getFittest().getGB(d,p)+" ");
		System.out.println();
		System.out.println("-----------------------------");
		System.out.println();
		...
		for (int d = 0; d < D; d++){
			for (int p = 0; p < P; p++) {
					Adp[d][p] =pop.getFittest().getGA(d,p);
			}
		}
		...
		for (int d = 0; d < D; d++) {
			...
			for (int i = 0; i < pop.getFittest().GBsize1(d); i++){
					if(pop.getFittest().getGB(d,i)!=0)
						B.add(pop.getFittest().getGB(d, i));
						B_position.add(i);
				}

...
public class PSO2_P {
	...
	Original_temp_variables_Global_best_Y.add(Y1);
		...
		Output_Result.PSO2_P(Y, X, Fittest, Fittest_Iter,T_total_time, (T_total_time-time_write_to_TXT),str);
		String X1="",Y1="";
		for(int i=0;i<X.length;i++)
				X1+=X[i]+" ";
		for(int i=0;i<Y.length;i++)
		...
		//	for(int t=0;t<100;t++){
			...
			//Output_Result.PSO2_P_Batch(Y_Batch, X_Batch, Fittest_Batch, generationCount_Batch,Time_Batch);
			//Output_Result.savePSO2_P_Batch_Parameter(populationSize_Batch,w_Batch,c1_Batch,c2_Batch, Vmax, Execution_Mode, Print_Mode, threshold );
		}
	}

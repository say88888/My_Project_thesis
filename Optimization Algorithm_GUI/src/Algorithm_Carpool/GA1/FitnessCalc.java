...
public class FitnessCalc extends GA1{
	...
	static double Fitness1(int[][] X, int[][] Y) {
		...
		for (int i = 0; i < D; i++)
			R3[i] = new int[Read_file.getJd(i)];

		for (int i = 0; i < D; i++)
			R4[i] = new int[Read_file.getJd(i)];

		for (int i = 0; i < R1.length; i++)
			R1[i] = GA_Read_Write_txt.R1();
		R2 = GA_Read_Write_txt.R2();
		for (int i = 0; i < R3.length; i++)
			for (int j = 0; j < R3[i].length; j++)
				R3[i][j] = GA_Read_Write_txt.R3();

		for (int i = 0; i < R4.length; i++)
			for (int j = 0; j < R4[i].length; j++)
				R4[i][j] = GA_Read_Write_txt.R4();

		for (int i = 0; i < R5.length; i++)
			R5[i] = GA_Read_Write_txt.R5();
		for (int k = 0; k < K; k++) {
			int s1 = 0, s2 = 0;
			for (int d = 0; d < D; d++)
				for (int j = 0; j < X[d].length; j++)

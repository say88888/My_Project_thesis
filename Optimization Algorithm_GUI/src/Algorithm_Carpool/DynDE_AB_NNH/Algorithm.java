...
public class Algorithm extends DynDE_AB_NNH {
	...
	public static Population evolvePopulation(Population pop) {
		...
		for (int i = 0; i < n3.size(); i++) {
			...
			for (int j = 0; j < n3.get(i).size(); j++) {
				...
				if (invid1.getFitness1() > Plbestinvid1.getFitness()) {	
					for (int d = 0; d < invid1.Xsize(); d++)
						Plbestinvid1.setPx(d, invid1.getX(d));
					for (int d = 0; d < invid1.Ysize(); d++)
						Plbestinvid1.setPy(d, invid1.getY(d));
					
					for (int d = 0; d < invid1.Asize(); d++)
						Plbestinvid1.setPA(d, invid1.getA(d));
					for (int d = 0; d < invid1.Bsize(); d++)
						Plbestinvid1.setPB(d, invid1.getB(d));
					
					for (int d = 0; d < invid1.binaryXsize(); d++)
						Plbestinvid1.setbinaryPx(d, invid1.getbinaryX(d));
					for (int d = 0; d < invid1.binaryYsize(); d++)
		...
		for (int i = 0; i < n3.size(); i++) {
			...
			if (Plbestinvid1.getFitness() > Ginvid1.getFitness1()) {
				...
				for (int d = 0; d < Ginvid1.Gxsize(); d++)
					Ginvid1.setGx(d, Plbestinvid1.getPx(d));
				for (int d = 0; d < Ginvid1.Gysize(); d++)
					Ginvid1.setGy(d, Plbestinvid1.getPy(d));
				
				for (int d = 0; d < Ginvid1.GAsize(); d++)

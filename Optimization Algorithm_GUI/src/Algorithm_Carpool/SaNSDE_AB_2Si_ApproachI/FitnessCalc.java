package Algorithm_Carpool.SaNSDE_AB_2Si_ApproachI;
...
public class FitnessCalc extends SaNSDE_AB_2Si_ApproachI {
	
	private static String[] t_GetOn=new String[P];
	private static String[] t_GetOff=new String[P];
	...
	static double getFitness1(Individual individual) {
		...
		// U1(X,Y)
			...
			double Sfmin;
			Sfmin = SaNSDE_AB_2Si_ApproachI.Sfmin;
	...
	static double getUFitness(U_Individual individual) {
		...
		for (int d = 0; d < D; d++) {
			List<Integer> B = new ArrayList<Integer>();
			for (int i = 0; i < individual.UBsize(); i++)
				for(int j=0;j<Dlist.get(d).size();j++){
					if (Math.abs(individual.getUB(i)) == (Dlist.get(d).get(j)))
							B.add(individual.getUB(i));
					}
			Bdm.add(B);
		}
	...
	static double U1(int[][] X, int[][] Y, int[][] Adp,
		...
		//(2-19)
		for(int p=0;p<P;p++) {
		if(t_GetOff[p]!=null || t_GetOff[p]!=null) {
			try {
				fitness+=Math.min(c1.reducetime(t_GetOff[p],t_GetOff[p]),0.0);
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	static boolean constraint(int[][] X, int[][] Y, int[][] Adp, List<List<Integer>> Bdm, int[][][] Cdmp, String[][] Tdm) {
		...
		//(2-19)
		 for(int p=0;p<P;p++){
			 boolean time = true;
			 if(t_GetOn[p]!=null || t_GetOff[p]!=null)
			 {
				 try {
					time=c1. time(t_GetOn[p],t_GetOff[p]);
				} catch (ParseException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			 }
			 if (time==false) { right=false;}
			 }
	...
	static String[][] Tdm(List<List<Integer>> Bdm)  {
		...
		t_GetOn=new String[P];
		t_GetOff=new String[P];
		...
		for (int d = 0; d < Tdm.length; d++) {
			...
			for (int m = 1; m < Tdm[d].length-1; m++) {
				if (Bdm.get(d).get(m-1) > 0) {
					...
					t_GetOn[Bdm.get(d).get(m-1)-1]=Driving_time;
					location1 = location2;
					...
					t_GetOff[-(Bdm.get(d).get(m-1))-1]=Driving_time;
					location1 = location2;
				}
			}
			location2 = Integer.parseInt(Read_INPUT_TXT_filter.getD_D_ADDRESS(d));

...
public class Population extends SaNSDE_AB_2Si_ApproachI {
	...
	public Individual getFittest() {
		...
		for (int i = 0; i < size(); i++) {
			...
			if(getIndividual(i).getFitness1()>Global_fittest){
				...
				for(int d=0;d<getIndividual(i).binaryXsize();d++)
					GX[d]=getIndividual(i).getbinaryX(d);
				for(int d=0;d<getIndividual(i).binaryYsize();d++)
					GY[d]=getIndividual(i).getbinaryY(d);
				
				for(int d=0;d<getIndividual(i).Asize();d++)
					GA[d]=getIndividual(i).getA(d);
				for(int d=0;d<getIndividual(i).Bsize();d++)
					GB[d]=getIndividual(i).getB(d);
				
				for(int d=0;d<getIndividual(i).Xsize();d++)
					Grid[d]=getIndividual(i).getrid(d);
				for(int d=0;d<getIndividual(i).Ysize();d++)
					GSi[d]=getIndividual(i).getSi(d);
			}

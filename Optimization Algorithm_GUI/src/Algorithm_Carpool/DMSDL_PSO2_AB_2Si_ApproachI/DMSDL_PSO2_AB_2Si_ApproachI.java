package Algorithm_Carpool.DMSDL_PSO2_AB_2Si_ApproachI;
...
public class DMSDL_PSO2_AB_2Si_ApproachI {
	...
	public static void main() throws IOException {
		...
		if (GUI.main.getmethod() != 2) {
			...
			R = PSO_Read_Write_txt.R();
			L = PSO_Read_Write_txt.L();
			F = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
			CR = PSO_Read_Write_txt.random_generate_or_read2(Execution_Mode);
			...
			for (int iter = 1; iter < Iteration; iter++) {
				...
				System.out.println("  F2(x,y) Fittest: = " + myPop.getFittest().getFitness1()
						...
						+ "     限制式 : " + CheckConstraint.CheckConstraints_boolean(GA,GB));
			//	L_right = false;
				if (Iteration_Mode != 2) {

					if ((Math.abs(myPop.getFittest().getFitness1() - myPopFitnessN_1)
							/ Math.abs(myPopFitnessN_1) > threshold) || CheckConstraint.CheckConstraints_boolean(GA,GB) == false
						...
						if (iter < Iteration * 0.9)
							...
							myPop = Algorithm.evolvePopulationafter(myPop);
						endTime = System.currentTimeMillis();
						SFminsb = new StringBuilder();
						PSO_Data_print.print2(myPop, Print_Mode);
				...
				if ((iter % L) == 0) {
					for (int L_iter = 0; L_iter < Local_Search_Iteration; L_iter++) {
						myPop = Algorithm.UpdataPlbest(myPop);
					//	if (L_right == true)
					//		break;
					}
				}
			...
			Output_Result.DMSDL_PSO_AB_2Si_ApproachI(GY, GX, Fittest, Fittest_Iter, T_total_time, (T_total_time - time_write_to_TXT), str);
			String X1 = "", Y1 = "";
			for (int i = 0; i < GX.length; i++)
				X1 += GX[i] + " ";
			for (int i = 0; i < GY.length; i++)
				Y1 += GY[i] + " ";
			Output_EXCEL.main(D, P, Omega_1, Omega_2, Omega_3, Omega_4, Omega_5, Omega_6, Omega_7, Omega_8, X1, Y1,
					Fittest, Iteration, Fittest_Iter, T_total_time, (T_total_time - time_write_to_TXT),
					CheckConstraint.CheckConstraints_boolean(GA,GB));
			if (Iteration_Mode == 2)
			...
			for (populationSize = Read_PSO_Batch_Parameter
					.lowpopulationSize(); populationSize <= Read_PSO_Batch_Parameter
							.UppopulationSize(); populationSize += Read_PSO_Batch_Parameter.StpopulationSize()) {
				for (w = Read_PSO_Batch_Parameter.loww(); w <= Read_PSO_Batch_Parameter
					...
					for (c1 = Read_PSO_Batch_Parameter.lowc1(); c1 <= Read_PSO_Batch_Parameter
						...
						for (c2 = Read_PSO_Batch_Parameter.lowc2(); c2 <= Read_PSO_Batch_Parameter
							...
							CheckConstraint.CheckConstraints(GA,GB);
							Print_input.main(Print_Mode);
							Output_Result.savePSO2_Batch_Parameter(n, populationSize, w, c1, c2, Vmax, Execution_Mode,
									Print_Mode, Write_Mode, threshold);
							Output_Result.PSO2_Batch(n, Y, X, Fittest, generationCount, T_total_time,
	...
	static void Order() throws Exception {
		...
		try {
			FileReader PBr = new FileReader(
					I_Set_Paths_for_Current_Simulation_Example.path() + "\\Carpool\\Orders.txt");

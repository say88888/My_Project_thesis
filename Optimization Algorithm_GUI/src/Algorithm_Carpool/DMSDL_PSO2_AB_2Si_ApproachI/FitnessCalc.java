...
public class FitnessCalc extends DMSDL_PSO2_AB_2Si_ApproachI {
	...
	static double getFitness1(Individual individual) {
		...
		// U1(X,Y)
			...
			double Sfmin;
			Sfmin = DMSDL_PSO2_AB_2Si_ApproachI.Sfmin;
	...
	static double getGVFitness(GV_Individual individual) {
		...
		for (int d = 0; d < D; d++) {
			List<Integer> B = new ArrayList<Integer>();
			for (int i = 0; i < individual.GVBsize(); i++)
				for(int j=0;j<Dlist.get(d).size();j++){
					if (Math.abs(individual.getGVB(i)) == (Dlist.get(d).get(j)))
							B.add(individual.getGVB(i));
					}
			Bdm.add(B);
		}

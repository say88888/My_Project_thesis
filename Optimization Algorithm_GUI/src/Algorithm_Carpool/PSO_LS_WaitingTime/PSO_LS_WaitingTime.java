...
public class PSO_LS_WaitingTime {
	...
	public static int Vmax,DeltaMax;
	...
	public static double Omega_1,Omega_2,Omega_3,Omega_4,Omega_5,Omega_6,Omega_7,Omega_8,Omega_9,Omega_10;
	...
	public static void main(  ) throws IOException {
		...
		Calculate_DeltaMax();   // 新贈計算DeltaMax
		...
		Omega_9=Read_Objective_Weight.Omega_9();
		Omega_10=Read_Objective_Weight.Omega_10();
...
//	for(int t=0;t<100;t++){
		...
		Output_Result.PSO_LS_WaitingTime(B, A, Fittest, Fittest_Iter, T_total_time, (T_total_time-time_write_to_TXT),str);
	...
	private static void Calculate_DeltaMax()
	{
		DeltaMax=0;
		int D1,D2,P1,DDtime,DPtime;
		for(int i=0;i<D;i++){
			D1=Integer.parseInt(Read_INPUT_TXT_filter.getD_S_PLACE(i));
			D2=Integer.parseInt(Read_INPUT_TXT_filter.getD_D_ADDRESS(i));
			DDtime=Read_distance.getDistancetime(D1,D2);
			for(int j=0;j<P;j++){
				P1=Integer.parseInt(Read_INPUT_TXT_filter.getP_R_PLACE(j));
				DPtime=Read_distance.getDistancetime(D1,P1);
					if(DeltaMax<(DDtime-DPtime))
					DeltaMax=(DDtime-DPtime);
			}
		}

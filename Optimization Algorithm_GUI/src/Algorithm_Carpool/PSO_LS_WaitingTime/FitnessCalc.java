...
public class FitnessCalc extends PSO_LS_WaitingTime {
	...
	static double getFitness1(Individual individual) {
		...
		// U1(X,Y)
			...
			double Sfmin;
			Sfmin = PSO_LS_WaitingTime.Sfmin;
	...
	static double getEFitness1(E_Individual individual) {
		...
		for (int d = 0; d < D; d++) {
			List<Integer> B = new ArrayList<Integer>();
			for (int i = 0; i < individual.EBsize(); i++)
				for(int j=0;j<Dlist.get(d).size();j++){
					if (Math.abs(individual.getEB(i)) == (Dlist.get(d).get(j)))
							B.add(individual.getEB(i));
					}
			Bdm.add(B);
		}
	...
	static double Fitness(int[][] X, int[][] Y, List<List<Integer>> Bdm,int[][] Adp) {
	...
	static double U1(int[][] X, int[][] Y, int[][] Adp,
		...
		for(int p=0;p<P;p++){
			...
			fitness += Math.min(Read_INPUT_TXT_filter.getseat(p) - s1p, 0.0);
		}
		...
		for(int p=0;p<P;p++){
			...
			for(int d=0;d<D;d++)
				Q1p+=Q1dp[d][p];
			fitness += Math.min(Q1p-Read_INPUT_TXT_filter.getseat(p) ,0.0);
		}
		...
		for(int p=0;p<P;p++){
			...
			for(int d=0;d<D;d++)
				Q2p+=Q2dp[d][p];
			fitness += Math.min(Q2p-Read_INPUT_TXT_filter.getseat(p) ,0.0);
		}
		
		//(2-5a-d)
		for (int d = 0; d < D; d++){
				double Qdkm=0;
				for (int m = 0; m < Bdm.get(d).size(); m++){
					if(Bdm.get(d).get(m)>0){
						int k=Bdm.get(d).get(m);
						Qdkm=Qdkm+Read_INPUT_TXT_filter.getseat(k-1);
						}
						if(Bdm.get(d).get(m)<0){
							int k=-(Bdm.get(d).get(m));
						Qdkm=Qdkm-Read_INPUT_TXT_filter.getseat(k-1);
				...
				fitness += Math.min(Qdkm ,0.0);
				fitness += Math.min(Qdkm-Read_INPUT_TXT_filter.getP_NUMBER(d) ,0.0);
			}
		...
		for (int d = 0; d < D; d++){
			...
			for (int m = 0; m < Bdm.get(d).size(); m++){
				if(Bdm.get(d).get(m)>0){
					int k=Bdm.get(d).get(m);
				WQdkm=WQdkm+Read_INPUT_TXT_filter.getweight(k-1);
				}
				if(Bdm.get(d).get(m)<0){
					int k=-(Bdm.get(d).get(m));
				WQdkm=WQdkm-Read_INPUT_TXT_filter.getweight(k-1);
			...
			fitness += Math.min(WQdkm ,0.0);
			fitness += Math.min(WQdkm-Read_INPUT_TXT_filter.getWeight_Limit(d) ,0.0);
		}
	...
	static boolean constraint(int[][] X, int[][] Y, int[][] Adp, List<List<Integer>> Bdm, int[][][] Cdmp, String[][] Tdm) {
		...
		for(int p=0;p<P;p++)
		{
			s212=Read_INPUT_TXT_filter.getseat(p);
			for (int d = 0; d < D; d++)
			{
				s211+=Q1dp[d][p];
			}
		...
		double s221a=0,s222a;
		for(int p=0;p<P;p++)
		{
			s222a=Read_INPUT_TXT_filter.getseat(p);
			for (int d = 0; d < D; d++)
			{
				s221a+=Q1dp[d][p];
			}
		...
		double s231a=0,s232a;
		for(int p=0;p<P;p++)
		{
			s232a=Read_INPUT_TXT_filter.getseat(p);
			for (int d = 0; d < D; d++)
			{
				s231a+=Q2dp[d][p];
			}
		...
		//(2-5a-d)
			for (int d = 0; d < D; d++){
				...
				for (int m = 0; m < Bdm.get(d).size(); m++){
					...
					if(Qdkm<0)
						right=false;
					if(Qdkm>Read_INPUT_TXT_filter.getP_NUMBER(d))
						right=false;
				}
			}
			...
			for (int d = 0; d < D; d++){
				...
				for (int m = 0; m < Bdm.get(d).size(); m++){
					...
					if(WQdkm<0)
						right=false;
					if(WQdkm>Read_INPUT_TXT_filter.getWeight_Limit(d))
						right=false;
				}
			}

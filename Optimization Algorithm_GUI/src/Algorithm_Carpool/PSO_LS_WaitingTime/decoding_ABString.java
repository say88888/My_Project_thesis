...
public class decoding_ABString extends PSO_LS_WaitingTime{
	...
	public	static void main(Population pop) throws IOException
	{
		...
		Output_Result.PSO_LS_WaitingTime_decodingABString(X,Y,Bids,Bdm);
	...
	static void decoding_ABString(Population pop)
	{
		...
		for (int d = 0; d < pop.getFittest().GAsize(); d++)
			System.out.print(pop.getFittest().getGA(d)+" ");
		System.out.println();
		System.out.println("-----------------------------");
		
		System.out.println("--------BString---------------------");
		for (int d = 0; d < pop.getFittest().GBsize(); d++)
			System.out.print(pop.getFittest().getGB(d)+" ");
		System.out.println();
		System.out.println("-----------------------------");
		System.out.println();
		...
		for (int d = 0; d < D; d++) {
			...
			List<Integer> B_position = new ArrayList<Integer>();
					for (int i = 0; i < pop.getFittest().GBsize(); i++)
						for(int j=0;j<Dlist.get(d).size();j++){
						if (Math.abs(pop.getFittest().getGB(i)) == (Dlist.get(d).get(j))){
							B.add(pop.getFittest().getGB(i));
							B_position.add(i);
						}
					}
			Bdm.add(B);

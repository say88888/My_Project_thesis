...
public class Check_DetourRation extends PSO_LS_WaitingTime{
		...
		for(int p=0;p<P;p++){
			...
			for(int d=0;d<D;d++){
				...
				if( ((Read_distance.getDistance(D_S,P_S)														//判定 司機起點到達乘客搭車點
						...
						<=10){       							//小於繞道值
				//	Read_filter_and_detour_rate.getdetour_rate()
					Alist2.add(d+1);																							//將司機加入Alist2
				}
			}
			if(Alist2.size()==0)																								//如果這位乘客沒有小於繞道值的司機
				Alist2.add(0);																									//將0加入Alist2
			Alist.add(Alist2);																									//將Alist2放進Alist

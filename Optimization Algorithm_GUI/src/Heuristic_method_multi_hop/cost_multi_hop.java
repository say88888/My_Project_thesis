...
public class cost_multi_hop extends Heuristic_multi_hop{
	...
	public static double Cost(int d) {
		...
		for(i=0;i<8;i++){
			...
			route+=Read_distance.getDistance(location1,location2);
			Dcost=Calculate_the_function.Cdj_cost(route,Read_INPUT_TXT_filter.getDriverOilConsumption(d),Read_Generate_random_OilConsumptionBounds.Driver_OilPrice(),"multi_hop",Qdjk);

...
public class step2_1 extends step2 {
	...
	public static void main()  {
		...
		int[] DP1=new int[select_Neighborhood_rule1.DP().get(d).size()];
		Order=new int[2*select_Neighborhood_rule1.DP().get(d).size()];
		Order1=new int[2*select_Neighborhood_rule1.DP().get(d).size()];
		distance=new double[2*select_Neighborhood_rule1.DP().get(d).size()];
		distance1=new double[2*select_Neighborhood_rule1.DP().get(d).size()];
		...
		for(int i=0;i<Order.length;i++){
		 ...
		 int location1= Integer.parseInt(Read_INPUT_TXT_filter.getD_S_PLACE(d));
		   
			System.out.println(" OK1 ");
			for(int i=0;i<select_Neighborhood_rule1.DP().get(d).size();i++)
			{
				DP1[i]=select_Neighborhood_rule1.DP().get(d).get(i);
			...
			System.out.println(" OK2 ");
			   ...
			   for(int i=0;i<DP1.length;i++)
			   {
				   ...
				   int Pon=Integer.parseInt(Read_INPUT_TXT_filter.getP_R_PLACE(DP1[i]-1));
				   int Poff=Integer.parseInt(Read_INPUT_TXT_filter.getP_D_ADDRESS(DP1[i]-1));

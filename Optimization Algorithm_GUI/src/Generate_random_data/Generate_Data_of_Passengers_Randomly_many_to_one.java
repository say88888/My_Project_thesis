...
public class Generate_Data_of_Passengers_Randomly_many_to_one {
	...
	public static void Generate_Data_of_Passengers_Randomly_many_to_one( int N,int M) throws Exception{
	...
	//-----------------------定義乘客相關資料-------------------------
	    ...
	    double[][] P_R_LNG=new double[P][M];        String[] P_D_ADDRESS= new String[P];              //乘客搭車地點經度  //定義乘客目的地位置			              
		...
		for(int i=0;i<P;i++)
		{
			...
			System.out.println("乘客  "+ (i+1));
					...
					P_n[i]=random;
					P_D_LAT[i]=Read_Generate_random.lat(random);     P_D_LNG[i]=Read_Generate_random.lng(random);      //乘客搭車地點緯度        //乘客搭車地點經度
					System.out.println("OK1 迄點為 = " + P_n[i]);
				
					if((int)Read_distance.getDistancetime(location2, P_n[i])>=0 && (int)Read_distance.getDistancetime(location2, P_n[i]) < 10){b=true;}
					}
			...
			ok: while(n1[i]>l && a==false){		
				...
				System.out.println("OK2 Qdjk = " + Qdjk);
						...
						System.out.println("OK3 起點為 = " + P_m[i][l]);
						
						P_R_LAT[i][l] =Read_Generate_random.lat(random1);          P_R_LNG[i][l]=Read_Generate_random.lng(random1);        //乘客目的地經度   //乘客目的地緯度
					
						time=(int) Read_distance.getDistancetime(P_m[i][l],P_n[i]);
						
						// 這邊是判別司機與乘客距離的遠近--------------------
						P_distance=Read_distance.getDistance(P_m[i][l],P_n[i]);
					
						Route=Read_distance.getDistance(location1,P_m[i][l])+P_distance+Read_distance.getDistance(P_n[i],location2);
					 
						Order[k]=P_m[i][l];  Order[k+1]=P_n[i];
						distance[k]=Read_distance.getDistance(location1,P_m[i][l]);  distance[k+1]=Read_distance.getDistance(location1,P_n[i]); 
				...
				for(int j=0;j<n1[i];j++)
					{
						...
						time=(int) Read_distance.getDistancetime(location1,P_m[i][j]);
						
						P_R_LAT[i][j] =Read_Generate_random.lat(P_m[i][j]);          P_R_LNG[i][j]=Read_Generate_random.lng(P_m[i][j]);        //乘客目的地經度   //乘客目的地緯度
						
						Driving_time=c1.Addtime(Driving_time,time);
						
						P_F_R_Time[i][j]=Driving_time;
						P_D_R_Time[i][j]=Driving_time;
						
						fph[i][j]=Calculate_the_function.Fph_cost(Read_distance.getDistance(P_m[i][j],P_n[i]),PassengerOilConsumptionBounds[i],Read_Generate_random_OilConsumptionBounds.Passenger_OilPrice());
						
						location1=P_m[i][j];
					
						P_F_A_Time[i]=c1.Addtime(P_F_R_Time[i][j],Read_distance.getDistancetime(location1,P_n[i]));
						P_D_A_Time[i]=c1.Addtime(P_D_R_Time[i][j],Read_distance.getDistancetime(location1,P_n[i]));
					}	

...
public class Generate_Data_of_Passengers_Randomly_one_to_one {
	...
	public static void Generate_Data_of_Passengers_Randomly(int MOD) throws IOException{
	...
	//-----------------------定義乘客相關資料-------------------------
		...
		{	
			...
			P_n[i]=random1;
			P_D_LAT[i] =Read_Generate_random.lat(random1);          P_D_LNG[i]=Read_Generate_random.lng(random1);        //乘客目的地經度   //乘客目的地緯度
			time=(int) Read_distance.getDistancetime(P_m[i],P_n[i]);
			if(time>10){a=true;}
			}
			
			P_F_A_Time[i]=Calculate_the_function.randomtime((Read_Generate_random.PTimeRandom(3)+time),(Read_Generate_random.PTimeRandom(2)+time));     //乘客到達目的時間
			P_D_A_Time[i]=Calculate_the_function.randomtime((Read_Generate_random.PTimeRandom(3)+time)+10,(Read_Generate_random.PTimeRandom(2)+time)+10);     //乘客延遲到達目的地時間

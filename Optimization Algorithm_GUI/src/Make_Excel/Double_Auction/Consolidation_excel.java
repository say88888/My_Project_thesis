...
public class Consolidation_excel {
	...
	public static void main( ) {
		...
		try {
			Workbook book = Workbook.getWorkbook(new File("output\\test.xls"));
			// 獲得第一個工作表對象
			Sheet sheet = book.getSheet(0);

			for (int j = 2; j <= sheet.getRows(); j += 2) {
				...
				cell1 = sheet.getCell(8, 1);
				result = cell1.getContents(); // 獲得單元格的內容
				test_data.add(result);
				cell1 = sheet.getCell(8, j);
				result = cell1.getContents(); // 獲得單元格的內容
				test_data.add(result);
				cell1 = sheet.getCell(10, j);
				result = cell1.getContents(); // 獲得單元格的內容
				test_data.add(result);
				cell1 = sheet.getCell(8, j + 1);
				result = cell1.getContents(); // 獲得單元格的內容
				test_data.add(result);
				cell1 = sheet.getCell(10, j + 1);
				result = cell1.getContents(); // 獲得單元格的內容
				test_data.add(result);

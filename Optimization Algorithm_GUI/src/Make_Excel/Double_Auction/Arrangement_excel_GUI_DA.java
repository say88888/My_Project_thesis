...
public class Arrangement_excel_GUI_DA {
				...
				try {
					...
					for (int j = 1; j <= sheet.getRows(); j++) {
						...
						for (int i = 0; i < test_data.size(); i++) {
							if (title_data.get(i).contains("Fittest") || title_data.get(i).contains("T_total(ms)") || title_data.get(i).contains("T_net(ms)")) {
								jxl.write.Number label1 = new jxl.write.Number(i, row, Double.parseDouble(test_data.get(i)), Format());
								sheet1.addCell(label1);
								sheet1.setColumnView(i, cellView);
							}
							else if (title_data.get(i).contains("Iteration") || title_data.get(i).contains("Generation"))
							{

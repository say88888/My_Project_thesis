...
public class Read_Carpool_file {
	...
	public static void loadData( ) throws IOException {
		...
		try {
			FileReader PBr = new FileReader(I_Set_Paths_for_Current_Simulation_Example.path()+"\\Carpool\\Parameter.txt");
		...
		try {
			FileReader PBr = new FileReader(  I_Set_Paths_for_Current_Simulation_Example.path()+"\\Carpool\\PassengerBids.txt");
			BufferedReader br = new BufferedReader(PBr);
		...
		Vector<String[]> str_vec = new Vector<String[]>(); // 暫存
		try {
			FileReader PBr = new FileReader(I_Set_Paths_for_Current_Simulation_Example.path()+"\\Carpool\\"+X_Select_Algorithm_Type_to_Solve_the_Problem.method()+"\\Parameter.txt");
			BufferedReader br = new BufferedReader(PBr);
			while ((str = br.readLine()) != null) { // 每次讀一行
				str_vec.add(str.split("\\s")); // 將此行以空白(white space)切成字串陣列, 存入
												// Vector暫存

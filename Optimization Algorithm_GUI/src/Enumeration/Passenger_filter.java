	...
	public static void main() throws IOException {
		...
		for(int j=0;j<Read_INPUT_TXT.getD();j++){
			...
			if( ((Read_distance.getDistance(D_S,P_S)																		//判定 司機起點到達乘客搭車點
					...
					<= Read_filter_and_detour_rate.getfilter_rate()){													//比例是否低於 detour_rate.txt值	
				D.add(i+1);
				P_filter++;
				break;
			}
 	...
 	System.out.println(); 
	 	...
	 	make_SQL_input_random_filter.main(P_filter);
	}

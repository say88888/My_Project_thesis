...
public class Population extends Double_Auction_DE_Real {
	...
	public Population(int populationSize, boolean initialise) {
		...
		if (initialise) {
			...
			for (int i = 0; i < size(); i++) {
				...
				newV_Individual.rspreviousIndividual();
				newV_Individual.rbpreviousIndividual();
				newV_Individual.rpreviousIndividual();
				...
				newU_Individual.rsglobalIndividual();
				newU_Individual.rbglobalIndividual();
				newU_Individual.rglobalIndividual();
		...
		for (int i = 0; i < size(); i++) {
			...
			if(getIndividual(i).getFitness1()>Global_fittest){
				...
				if(Omega2==1 || Omega3 ==1){
					for(int d=0;d<getIndividual(i).rssize();d++)
						rs[d]=getIndividual(i).getrs(d);
					for(int d=0;d<getIndividual(i).rbsize();d++)
						rb[d]=getIndividual(i).getrb(d);
				}
				if(Omega4==1 )
					r=getIndividual(i).getr();
			}

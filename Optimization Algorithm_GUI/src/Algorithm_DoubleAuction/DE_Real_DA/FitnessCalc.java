...
public class FitnessCalc extends Double_Auction_DE_Real {
	...
	static double getFitness1(Individual individual) {
		...
		if(Omega1==1){
			...
			// U1(X,Y)
				...
				double Sfmin;
				Sfmin = Double_Auction_DE_Real.Sfmin;
	...
	static double getUFitness(U_Individual individual) {
		...
		for (int i = 0; i < I; i++)
			for (int j = 0; j < X[i].length; j++) {
				...
				rs[i][j] = individual.getUrs(b);
				b++;
			}

		double r = individual.getUr();

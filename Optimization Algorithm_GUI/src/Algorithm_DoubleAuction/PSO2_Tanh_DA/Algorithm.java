...
public class Algorithm extends Double_Auction_PSO2_Tanh {
		...
		for (int i = 0; i < pop.size(); i++) {
			...
			for (int d = 0; d < invid.Xsize(); d++) {		//變更X粒子
				...
				if (Print_Mode == 1) {
					...
					System.out.println(" Tanh=" + Math.tanh(Math.abs(Vinvid.getVx(d)))   );
				}
				if (rsid > Math.tanh(Math.abs(Vinvid.getVx(d))) ) {
					if (Print_Mode == 1)
						System.out.print("rsid>Tanh  X為1");
					invid.setX(d, 1);
				} else {
					if (Print_Mode == 1)
						System.out.print("rsid<Tanh  X為0");
					invid.setX(d, 0);
				}
				if (Print_Mode == 1)
					System.out.println();
			...
			for (int d = 0; d < invid.Ysize(); d++) {	//變更Y粒子
				...
				if (Print_Mode == 1) {
					System.out.print("rsid=" + rsid);
					System.out.println(" Tanh=" + Math.tanh(Math.abs(Vinvid.getVy(d)))   );
				}
				if (rsid > Math.tanh(Math.abs(Vinvid.getVy(d)))) {
					if (Print_Mode == 1)
						System.out.print("rsid>Tanh  Y為1");
					invid.setY(d, 1);
				} else {
					if (Print_Mode == 1)
						System.out.print("rsid<Tanh  Y為0");
					invid.setY(d, 0);
				}
				if (Print_Mode == 1)
					System.out.println();

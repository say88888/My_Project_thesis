...
import Input_Output_txt.Double_Auction.Read_DE_Group_Size_DA;
...
//	public static ArrayList<Double> successfulCR = new ArrayList<Double>();
	...
	public static void main() throws IOException {
		...
		Read_DE_Group_Size_DA.main();
		S = new int[Read_DE_Group_Size_DA.S().size()];
		for (int i = 0; i < Read_DE_Group_Size_DA.S().size(); i++)
			S[i] = Read_DE_Group_Size_DA.S().get(i);
		n3 = new ArrayList<List<Integer>>();
		// 進行分群
		DE_Read_Write_txt.random_generate_or_read7(Execution_Mode);
		Type=new int[n3.size()];
		...
		Output_Result.DynDE(GY, GX, Fittest, Fittest_Iter,T_total_time, (T_total_time-time_write_to_TXT),str);
		String X1="",Y1="";
		for(int i=0;i<GX.length;i++)
			X1+=GX[i]+" ";
		for(int i=0;i<GY.length;i++)

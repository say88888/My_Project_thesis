...
public class Double_Auction_LR_and_PSO2_P {
 ...
 */public static int Iteration;
	...
	//	Lambda3 = new double[I][K];
		...
		L=Read_LR_and_PSO2_P_Parameter.L();
		c=Read_LR_and_PSO2_P_Parameter.c();
		Print=Read_LR_and_PSO2_P_Parameter.Print();
		...
		*/
		...
		Output_Result.LR_and_PSO2_P(Y, X, Fittest, Fittest_Iter, T_total_time, (T_total_time-time_write_to_TXT));
		String X1="",Y1="";
		for(int i=0;i<X.length;i++)
			for(int j=0;j<X[i].length;j++)
				X1+=X[i][j]+" ";
		...
		//	for(int t=0;t<100;t++){
			...
			Output_Result.saveLR_and_PSO2_P_Batch_Parameter(n,populationSize,w,c1,c2, Vmax, Execution_Mode, Print_Mode,Write_Mode, threshold );
			Output_Result.LR_and_PSO2_P_Batch(n,Y, X, Fittest, generationCount,T_total_time, (T_total_time-time_write_to_TXT));
			System.out.println();

...
public class Double_Auction_NSDE {
...
//	public static ArrayList<Double> successfulCR = new ArrayList<Double>();
	...
	//	CheckConstraint.CheckConstraints();
		if(Omega1==1)
			Output_Result.NSDE_DA(GY, GX, Fittest, Fittest_Iter, T_total_time,
					(T_total_time - time_write_to_TXT), str);
		else if(Omega2==1 || Omega3==1)
			Output_Result.NSDE_DA_problem2(GY, GX, rb,rs,Fittest, Fittest_Iter, T_total_time,
					(T_total_time - time_write_to_TXT), str);
		else if(Omega4==1)
			Output_Result.NSDE_DA_problem4(GY, GX, r,Fittest, Fittest_Iter, T_total_time,
					(T_total_time - time_write_to_TXT), str);
		String X1="",Y1="",rs1="",rb1="",r1="";
		for(int i=0;i<GX.length;i++)
				X1+=GX[i]+" ";

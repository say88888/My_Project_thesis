...
public class Double_Auction_L_SHADE_CCPSO2 {
	...
	public static int H,k11;
	public static double[] MAlpha1,MAlpha2,MAlpha3;
	public static List<Double> SAlpha1 = new ArrayList<Double>();
	public static List<Double> SAlpha2 = new ArrayList<Double>();
	public static List<Double> SAlpha3 = new ArrayList<Double>();
	public static List<Double> ferc = new ArrayList<Double>();
	...
	public static void main() throws IOException {
		...
		k11=0;
		SAlpha1 = new ArrayList<Double>();
		SAlpha2 = new ArrayList<Double>();
		SAlpha3 = new ArrayList<Double>();
		ferc = new ArrayList<Double>();
		Sfmin=0;
		...
		if (GUI.main.getmethod() != 2) {
			...
			PSO_lamda= PSO_Read_Write_txt.threshold();
			Alpha1=new double[populationSize];
			Alpha2=new double[populationSize];
			Alpha3=new double[populationSize];
			
			H=PSO_Read_Write_txt.H();
			MAlpha1=new double[H];
			MAlpha2=new double[H];
			MAlpha3=new double[H];
			for(int i=0;i<H;i++){
				MAlpha1[i]=PSO_Read_Write_txt.Alpha1();
				MAlpha2[i]=PSO_Read_Write_txt.Alpha2();
				MAlpha3[i]=PSO_Read_Write_txt.Alpha3();
			}
		...
		//		 System.out.println("  F2(x,y) Fittest: = "+ myPop.getFittest().getFitness1() +"   F2(x,y) Fittest 成長值 = "+Math.abs(myPop.getFittest().getFitness1()-myPopFitnessN_1)/Math.abs(myPopFitnessN_1)+"     限制式 : " + constraint(X, Y));
			...
			if(Omega1 == 1)
				Output_Result.L_SHADE_CCPSO2(Y, X, Fittest, Fittest_Iter, T_total_time,
						(T_total_time - time_write_to_TXT));
			else if(Omega2 == 1 || Omega3 == 1)
				Output_Result.L_SHADE_CCPSO2_problem2(Y, X,rb,rs, Fittest, Fittest_Iter, T_total_time,
						(T_total_time - time_write_to_TXT));
			else if(Omega4 == 1)
				Output_Result.L_SHADE_CCPSO2_problem4(Y, X,r, Fittest, Fittest_Iter, T_total_time,
						(T_total_time - time_write_to_TXT));
			String X1 = "", Y1 = "", rs1 = "", rb1 = "",r1="";
			for (int i = 0; i < X.length; i++)
				X1 += X[i] + " ";

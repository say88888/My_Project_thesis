...
public class U_Individual extends Double_Auction_SaNSDE_Problem3 {

	static int defaultGeneLengthX = Xsize;
	private double[] Ux = new double[defaultGeneLengthX];
	private int[] IntegerUx = new int[defaultGeneLengthX];
	...
	public void XglobalIndividual() {
		...
		IntegerUx = new int[defaultGeneLengthX];
		for (int i = 0; i < Uxsize(); i++) {
			Ux[i] = DE_Read_Write_txt.random_generate_or_read6(Execution_Mode,Read_Sik_Parameter.top());
			double rsid = DE_Read_Write_txt
					.random_generate_or_read1(Execution_Mode);
			if (rsid <R) {
				IntegerUx[i] = (int)Math.floor(Ux[i]);
			} else {
				IntegerUx[i] = (int)Math.ceil(Ux[i]);
			}
		}
	}
	...
	public int getIntegerUx(int index) {
		return IntegerUx[index];
	}
	...
	public void setIntegerUx(int index, int value) {
		IntegerUx[index] = value;
		fitness = 0;
		fitness1 = 0;
	}

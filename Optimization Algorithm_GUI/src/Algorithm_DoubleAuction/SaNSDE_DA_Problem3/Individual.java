...
public class Individual extends Double_Auction_SaNSDE_Problem3 {

	static int defaultGeneLengthX = Xsize;
	private double[] X = new double[defaultGeneLengthX];
	private int[] IntegerX = new int[defaultGeneLengthX];
	...
	public void XgenerateIndividual() {
		...
		IntegerX = new int[defaultGeneLengthX];
		for (int i = 0; i < Xsize(); i++) {
			X[i] = DE_Read_Write_txt.random_generate_or_read6(Execution_Mode,Read_Sik_Parameter.top());
			double rsid = DE_Read_Write_txt
					.random_generate_or_read1(Execution_Mode);
			if (rsid < R) {
				IntegerX[i] = (int)Math.floor(X[i]);
			} else {
				IntegerX[i] = (int)Math.ceil(X[i]);
			}
		}
	}
	...
	public int getIntegerX(int index) {
		return IntegerX[index];
	}
	...
	public void setIntegerX(int index, int value) {
		IntegerX[index] = value;
		fitness = 0;
		fitness1 = 0;
	}

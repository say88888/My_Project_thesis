...
public class FitnessCalc extends Double_Auction_SaNSDE_Problem3 {
			...
			for(int n=0;n< X[i].length;n++)
				for(int h=0;h<X[i][n].length;h++)
					for(int k=0;k<X[i][n][h].length;k++)
						X[i][n][h][k]= individual.getIntegerX(b++);
	...
	//U1(X,Y)
		...
		double Sfmin;
		Sfmin=Double_Auction_SaNSDE_Problem3.Sfmin;
		 ...
		 int[][] Y = new int[N][];
			...
			for (int i = 0; i < I; i++)
				for(int n=0;n< X[i].length;n++)
					for(int h=0;h<X[i][n].length;h++)
						for(int k=0;k<X[i][n][h].length;k++)
							X[i][n][h][k]= individual.getIntegerUx(b++);

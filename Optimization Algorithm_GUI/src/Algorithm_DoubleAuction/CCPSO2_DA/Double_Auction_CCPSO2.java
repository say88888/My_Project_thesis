...
public class Double_Auction_CCPSO2 {
	...
	public static void main() throws IOException {
		...
		//		 System.out.println("  F2(x,y) Fittest: = "+ myPop.getFittest().getFitness1() +"   F2(x,y) Fittest 成長值 = "+Math.abs(myPop.getFittest().getFitness1()-myPopFitnessN_1)/Math.abs(myPopFitnessN_1)+"     限制式 : " + constraint(X, Y));
			...
			if(Omega1 == 1)
				Output_Result.CCPSO2(Y, X, Fittest, Fittest_Iter, T_total_time,
						(T_total_time - time_write_to_TXT));
			else if(Omega2 == 1 || Omega3 == 1)
				Output_Result.CCPSO2_problem2(Y, X,rb,rs, Fittest, Fittest_Iter, T_total_time,
						(T_total_time - time_write_to_TXT));
			else if(Omega4 == 1)
				Output_Result.CCPSO2_problem4(Y, X,r, Fittest, Fittest_Iter, T_total_time,
						(T_total_time - time_write_to_TXT));
			String X1 = "", Y1 = "", rs1 = "", rb1 = "",r1="";
			for (int i = 0; i < X.length; i++)
				X1 += X[i] + " ";

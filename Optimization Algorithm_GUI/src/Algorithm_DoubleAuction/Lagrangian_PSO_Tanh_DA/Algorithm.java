...
public class Algorithm extends Double_Auction_ALPSO_Tanh {
	...
	public static Population evolvePopulation(Population pop) {
		...
		for (int i = 0; i < pop.size(); i++) {
			...
			for (int d = 0; d < Vinvid.Vxsize(); d++) {
				...
				Vinvid.setVx(d,
						w * Vinvid.getVx(d)
								+ (c1 * r1 * (Pinvid.getPx(d) - invid.getX(d)))
		...
		for (int i = 0; i < pop.size(); i++) {
			...
			for (int d = 0; d < invid.Xsize(); d++) { // 變更X粒子
				...
				if (Print_Mode == 1) {
					...
					System.out.println(" Tanh="
							+ Math.tanh(Math.abs(Vinvid.getVx(d))));
				}
			...
			for (int d = 0; d < invid.Ysize(); d++) { // 變更Y粒子
				...
				if (Print_Mode == 1) {
					System.out.print("rsid=" + rsid);
					System.out.println(" Tanh="
							+ Math.tanh(Math.abs(Vinvid.getVy(d))));
				}

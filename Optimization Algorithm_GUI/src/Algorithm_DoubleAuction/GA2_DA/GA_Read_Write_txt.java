...
public class GA_Read_Write_txt extends Double_Auction_GA2{
	...
	public static void read_GA_Parameters()throws IOException{
		...
		String line;
		int d=0;
		        FileReader GAPfr = new FileReader(I_Set_Paths_for_Current_Simulation_Example.path()+"\\GA_Parameter\\GA2\\GA_Parameter.txt");
	...
	{	
		try {
			BufferedReader in = new BufferedReader(new FileReader(I_Set_Paths_for_Current_Simulation_Example.path()+"\\GA_Parameter\\GA2\\evolution_log.txt"));
	...
	{
		try {
			FileWriter fw = new FileWriter(I_Set_Paths_for_Current_Simulation_Example.path()+"\\GA_Parameter\\GA2\\evolution_log.txt", false);
	...
	public static double read_from_evolution_log1()
	  {
	  	try {
	  		BufferedReader in = new BufferedReader(new FileReader(I_Set_Paths_for_Current_Simulation_Example.path()+"\\GA_Parameter\\GA2\\evolution_log1.txt"));
	  				String str;String [] y = null;
	  			//	double xxx1;
	  		while ((str = in.readLine()) != null) { // readLine()依序讀取檔案內的一行文字
	  		y=str.split(" ");
	...
	public static void write_to_evolution_log1(StringBuilder sb) 
	  {
	  	try {
	  		FileWriter fw = new FileWriter(I_Set_Paths_for_Current_Simulation_Example.path()+"\\GA_Parameter\\GA2\\evolution_log1.txt", false);
	  		PrintWriter pw = new PrintWriter(fw);
	  		pw.print(sb);
	  		pw.flush();
	  		pw.close();

...
public class Double_Auction_LRPSO_Tanh {
	...
	public static void main( ) throws IOException {
		...
		L=Read_LRPSO_Tanh_Parameter.L();
		c=Read_LRPSO_Tanh_Parameter.c();
		Print=Read_LRPSO_Tanh_Parameter.Print();
		...
		System.out.println(Functions.L(X, Y));
		...
		Output_Result.LRPSO_Tanh(Y, X, Fittest, Fittest_Iter,T_total_time, (T_total_time-time_write_to_TXT),str);
		String X1="",Y1="";
		for(int i=0;i<X.length;i++)
			for(int j=0;j<X[i].length;j++)
				X1+=X[i][j]+" ";
		...
		//	for(int t=0;t<100;t++){
			...
			System.out.println("執行時間扣除寫檔時間  T_net=" + (T_total_time-time_write_to_TXT)+"ms");
			CheckConstraint.CheckConstraints(X,Y);
			Output_Result.saveLRPSO_Tanh_Batch_Parameter(n,populationSize,w,c1,c2, Vmax, Execution_Mode, Print_Mode, threshold );
			Output_Result.LRPSO_Tanh_Batch(n,Y, X, Fittest, generationCount,T_total_time, (T_total_time-time_write_to_TXT));
			System.out.println();
